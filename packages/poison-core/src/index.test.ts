import { inject, injectSingle } from './index';
...
describe('inject', () => {
  ...
  it('should inject a comment into the body by default', () => {
    const poisonedHtml = inject(html, { payload, techniques: ['comment'] });
  ...
  it('should inject a zero-width watermark into the first <p> by default', () => {
    const poisonedHtml = inject(html, { payload, techniques: ['zerowidth'] });
    expect(poisonedHtml).not.toBe(html);
  });

  it('should inject a zero-width watermark into a specific element', () => {
    const poisonedHtml = inject(html, { payload, techniques: ['zerowidth'], selector: '#target' });
    expect(poisonedHtml).not.toBe(html);
  });

  it('should inject a JSON-LD script into the head by default', () => {
    const poisonedHtml = inject(html, { payload, techniques: ['jsonld'] });
...
describe('injectSingle', () => {
  ...
  it('should inject a single technique', () => {
    const poisonedHtml = injectSingle(html, { payload, technique: 'comment' });
    expect(poisonedHtml).toContain(`<!-- ${payload} -->`);
  });

  it('should work with selector', () => {
    const poisonedHtml = injectSingle(html, { payload, technique: 'css', selector: 'p' });
